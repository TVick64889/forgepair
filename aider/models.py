...
# These two lists back Model.fast_validate_environment()'s fast path for
# resolving which API-key env var a model needs, without forcing litellm's
# ~1.5s import for the common case. They are regenerated periodically from
# litellm's bundled model data by scripts/update_model_lists.py rather than
# hand-maintained -- run that script to refresh them; do not hand-edit the
# block below (see that script's docstring for why this isn't computed at
# runtime instead).
# --- BEGIN GENERATED FAST-PATH MODEL LISTS ---
...
# --- END GENERATED FAST-PATH MODEL LISTS ---
